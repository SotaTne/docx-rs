...
impl ElementReader for OMathAccent {
    fn read<R: Read>(
        r: &mut EventReader<R>,
            ...
            match r.next() {
                Ok(XmlEvent::StartElement {
                    ...
                    match OMathXMLElement::from_str(&name.local_name).unwrap() {
                        OMathXMLElement::AccentProperty => {
                            property = Some(OMathAccentProperty::read(r, &attributes)?);
                        }
                ...
                Ok(XmlEvent::EndElement { name, .. }) => {
                    if name.prefix.as_deref() == Some("m")
                        && OMathXMLElement::from_str(&name.local_name).unwrap()
                            == OMathXMLElement::Accent
                    {
                        return Ok(OMathAccent {
...
impl ElementReader for OMathAccentProperty {
    fn read<R: Read>(
        r: &mut EventReader<R>,
        _attrs: &[OwnedAttribute],
    ) -> Result<Self, ReaderError> {
        let mut property = OMathAccentProperty::default();
        loop {
            match r.next() {
                Ok(XmlEvent::StartElement {
                    name, attributes, ..
