...
pub fn main() -> Result<(), DocxError> {
    ...
    let pre_sub_super_sample = OMathPara::new()
        .justification(OMathJustificationType::Center)
        .add_math(OMath::new().add_pre_sub_super(OMathPreSubSuper::new(
            OMathSubArgument::new().add_run(OMathRun::new().add_text("m")),
            OMathSuperArgument::new().add_run(OMathRun::new().add_text("n")),
            OMathBase::new().add_run(OMathRun::new().add_text("T")),
        )));
    ...
    let bar_simple = OMathPara::new()
        .justification(OMathJustificationType::Center)
        .add_math(OMath::new().add_bar(OMathBar::overline(
            OMathBase::new().add_run(OMathRun::new().add_text("x")),
        )));
    ...
    docx = add_tag_block(
        ...
        vec![math_paragraph(function_sin), math_paragraph(function_log)],
    );
    ...
    docx = add_tag_block(
        ...
        vec![Paragraph::new().add_run(Run::new().add_text(
            "This document writes math defaults such as Cambria Math, binary-break behavior, display defaults, default justification, spacing, wrap indent, and integral/n-ary limit locations into word/settings.xml. The API also supports wrapRight, but settings.xml can emit only one of wrapIndent or wrapRight at a time.",
        ))],
    );
